...
const ReportsPage = () => {
  ...
  const totalDays = approved.reduce((s, r) => s + Number(r.total_days), 0);
  ...
  const byDept = departments.map((d) => {
    ...
    const days = approved.filter((r) => deptEmps.some((e) => e.id === r.employee_id)).reduce((s, r) => s + Number(r.total_days), 0);
    return { name: d.name.length > 15 ? d.name.substring(0, 15) + "..." : d.name, days };
  });

  const byType = leaveTypes.map((lt) => ({
    name: lt.name,
    value: approved.filter((r) => r.leave_type_id === lt.id).reduce((s, r) => s + Number(r.total_days), 0),
  })).filter((d) => d.value > 0);
  ...
  const handleExport = () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = "bao-cao-nghi-phep.csv"; a.click();
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={byDept}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" fontSize={10} /><YAxis fontSize={12} /><Tooltip /><Bar dataKey="days" fill="#2563EB" radius={[4,4,0,0]} /></BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        <Card>
