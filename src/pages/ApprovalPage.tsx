...
const ApprovalPage = () => {
  ...
  const handleReject = async () => {
    if (!rejectId) return;
    await updateLeaveRequest(rejectId, { status: "rejected", rejected_reason: rejectReason, approved_by: currentUser?.employeeId });
            ...
            return (
              <div className="space-y-3 text-sm">
                <div className="grid grid-cols-2 gap-2">
                  <div><span className="text-muted-foreground">Họ tên:</span> <strong>{emp?.full_name}</strong></div>
