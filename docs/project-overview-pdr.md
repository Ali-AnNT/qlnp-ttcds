...
| **Current Version** | 0.7.0 (Configurable work days, MyStats, Docker, auth token renewal) |
...
- Tu dong tinh so ngay nghi (configurable work days via SystemConfig `work_days`, default Mon-Fri)
...
- Backend ho tro xuat Excel `.xlsx` (ClosedXML); frontend hien thi bao cao voi Recharts

### 4.6 MyStats (All Roles)
- GET /api/my-stats/ returns: RemainingDays, PendingCount, ApprovedCount, UsedDays
- Lazy-seed: ensure balance rows exist before computing stats
- Aggregate balance totals + leave request status counts for current user

### 4.7 Giam Sat Vuot Muc Quy Dinh (GD.PGD)
...
### 4.8 Cau Hinh He Thong (QTHT)
- Tab Cau hinh chung: 9 configurable system settings stored in SystemConfigs key-value table (max_annual_leave, min_request_days, max_carry_over, leave_cycle, work_days, default_days per role: CB.PCM, LD.PCM, GD.PGD, QTHT). QTHT-only write access via PUT /api/system-configs
...
| **Availability** | 99% trong gio hanh chinh (8h-17h, configurable work days) |
| **Security** | JWT Bearer auth (SSO Portal delegates), ICurrentUserProvider (claims-based), role-based endpoint authorization, 401-reactive token renewal with dedup |
...
- Infrastructure: .NET 10 API server + SQL Server (on-premise), Docker deployment supported
- Backend: FastEndpoints v8.1.0 + Vertical Slice Architecture + EF Core 9 + SQL Server
- Frontend: SPA React 18, khong SSR, Vite 5 build tool
- Database: SQL Server (existing VI_NGHIPHEP database)
- Auth: JWT Bearer (SSO Portal issues token, iframe embeds via postMessage). API resolves via ICurrentUserProvider reading claims. Dev mode uses `/api/auth/dev/login` to issue local test JWTs. 401-reactive token renewal via SSO refresh endpoint with dedup lock
- Hosting: Docker Compose (api:8003, web:8001), or IIS (API reverse proxy) + static hosting (frontend)
...
| TanStack React Query | ^5.83.0 | Server state caching (100% migrated, no Zustand) |
| React Router | ^7.16.0 | SPA routing |
| Recharts | ^3.8.1 | Charts |
| React Hook Form | ^7.72.1 | Form handling |
| Zod | ^4.3.6 | Schema validation |
| date-fns | ^4.1.0 | Date utilities |
| react-day-picker | ^8.10.1 | Calendar date picker |
| shadcn/ui (Radix) | latest | UI component library (52 components) |
