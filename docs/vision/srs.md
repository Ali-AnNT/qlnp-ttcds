...
- **Backend:** Supabase (PostgreSQL + RPC + RLS), không server riêng
- **Styling:** Tailwind CSS 3 + shadcn/ui (Radix UI)
- **State Management:** Zustand 5 (client) + TanStack React Query 5 (server cache)
- **Auth:** Username/password qua RPC `verify_login`, không JWT tự quản
...
|----|-------------|----------|
| FR-01.1 | Login bằng username + password qua Supabase RPC `verify_login` | P0 |
| FR-01.2 | Auth state lưu trong Zustand, tự redirect về /login nếu chưa auth | P0 |
| FR-01.3 | Hiển thị thông tin user: họ tên, phòng ban, chức vụ, role trong header | P1 |
| FR-01.4 | Logout xóa state, redirect về login | P0 |
| FR-01.5 | Chỉ user có `is_active = true` mới đăng nhập được | P0 |

**Verification:** Test login với username/password đúng, sai, user bị disable.
...
- Auth: custom `verify_login()` RPC function (SECURITY DEFINER), so sánh password_hash trực tiếp (plaintext trong DB — KHÔNG AN TOÀN)
- Authorization: Row Level Security (RLS) tất cả các bảng
- Current policy: public read trên tất cả bảng, public insert/update trên leave_requests
- **Security gap:** RLS quá mở, dựa hoàn toàn vào front-end filtering. Không có row-level filtering theo role ở DB level.

### 4.4 Data Validation Rules

| Rule | Where Applied |
|------|---------------|
| start_date <= end_date | Frontend form validation |
| start_date >= today | Frontend form validation |
| reason not empty | Frontend form validation |
| total_days >= 1 | Frontend calculation |
| No overlap with approved dates | Frontend memo check |
| Status check constraint | Database level |
...
| Password hashing | **GAP:** password_hash stored/comparable as plaintext |
| RLS per role | **GAP:** RLS policies are too permissive (public read all) |
| HTTPS | Dependent on Supabase + hosting |
| Input sanitization | Not explicitly handled |
...
|----|-----------|---------------------|---------------|
| FR-01 | Unit + Manual | Login valid/invalid/disabled user | Đúng redirect, đúng error msg |
...
### 6.3 Known Gaps (for verification)

| Gap | Severity | Recommendation |
|-----|----------|----------------|
| Password plaintext comparison | **HIGH** | Hash passwords with bcrypt, update `verify_login` RPC |
| RLS policies quá mở | **HIGH** | Implement proper RLS policies based on `auth.uid()` or app user claim |
| Không có CSRF protection | **MEDIUM** | Add CSRF token cho mutation endpoints |
| Không có rate limiting trên login | **MEDIUM** | Implement login attempt tracking / cooldown |
| Không có audit log | **LOW** | Log approval/rejection actions với timestamp + actor |
| Định mức 12 hardcoded trong frontend | **MEDIUM** | Move LIMIT vào `leave_config` table |
| Không có pagination | **LOW** | Data lớn sẽ chậm load; add server-side pagination |
| Dùng approved_by làm approver ID | **MEDIUM** | Field này dùng để chỉ định người duyệt tiềm năng nhưng cũng bị overwrite khi duyệt; tách thành 2 fields riêng |
