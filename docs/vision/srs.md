...
| RLS | Row Level Security — historical (Supabase PostgreSQL), now replaced by application-level auth |
| Business days | Ngày làm việc (T2-T6), dùng để tính số ngày nghỉ |
| Định mức | Ngày phép mặc định từ `LeaveType.DefaultDays`, tracked trong `LeaveBalances` |
...
- **Backend:** .NET 10 + FastEndpoints + Vertical Slice Pattern (VSP) + EF Core 9 + SQL Server
...
- **Auth:** JWT Bearer. SSO Portal issues JWT, frontend nhận qua postMessage (iframe) hoặc localStorage. API validates JWT via symmetric key, resolves current user qua `ICurrentUserProvider` reading claims; dev mode có anonymous fallback admin local
...
| FR-01.7 | `ICurrentUserProvider` đọc JWT claims để resolve CurrentUser (UserId, DisplayName, Roles, UnitId, PhongBanId, ...); JWT validate qua symmetric key (Jwt:SigningKey) | P0 |
| FR-01.8 | Dev mode fallback trả user admin local khi anonymous (no JWT) | P1 |

**Verification:** Test iframe postMessage token, `/api/auth/me` với JWT hợp lệ/thiếu, dev mode anonymous fallback, logout clear token.
...
| FR-02.5 | Ngày phép còn lại = LeaveBalance.TotalDays - LeaveBalance.UsedDays (nguồn truth: LeaveType.DefaultDays, không hardcode 12) | P0 |
...
| ID | Requirement | Priority |
|----|-------------|----------|
| FR-09.1 | Định mức từ `LeaveBalance.TotalDays` (nguồn truth: `LeaveType.DefaultDays`), không hardcode 12 | P0 |
...
| FR-09.7 | Bảng cấp phòng ban: tên phòng, tổng CB, CB vượt mức, tổng ngày đã duyệt, định mức (= tổng CB × LeaveType.DefaultDays), vượt, nút xem chi tiết | P0 |
...
- Auth: JWT Bearer — SSO Portal issues JWT, frontend nhận token qua postMessage (iframe) hoặc Authorization header, API validates JWT qua symmetric key
- Current user: `ICurrentUserProvider` đọc JWT claims (UserId, DisplayName, Roles, UnitId, PhongBanId, ...), lookup `USER_MASTER` + `UserRoles`, returns `CurrentUser` record
- Password: QLNP không lưu hoặc xác minh password người dùng; auth thuộc hệ thống SSO Portal
- Authorization: Role-based trong từng FastEndpoints endpoint — kiểm tra current user + role trước khi thực thi business logic
- Middleware: JWT Bearer authentication resolves user via ICurrentUserProvider trước khi FastEndpoints xử lý request
...
| Gateway/JWT authentication | JWT Bearer + ICurrentUserProvider resolve user từ JWT claims |
| Role-based API authorization | Endpoint-level guard + role checks per slice |
| JWT authentication | JWT Bearer validation (Issuer, Audience, SigningKey), ClaimsPrincipal-based |
...
|----|-----------|---------------------|---------------|
| FR-01 | Integration + Manual | GET /api/auth/me với JWT hợp lệ/thiếu, postMessage token, dev anonymous fallback | Current user đúng, lỗi auth khi thiếu context |
...
├── Middleware/
│   └── CurrentUser.cs                 # CurrentUser record (from JWT claims)
├── Auth/
│   ├── ICurrentUserProvider.cs        # Interface for current user resolution
│   └── CurrentUserProvider.cs         # Reads claims from JWT
├── Data/
