...
**Version:** 1.0 | **Date:** 2026-05-12 | **Author:** Generated from codebase analysis
...
- **Backend:** .NET 9 + FastEndpoints + Vertical Slice Architecture + Dapper + SQL Server
- **Architecture Pattern:** Vertical Slice Architecture — code tổ chức theo feature (Login, CreateLeave, ApproveLeave...) thay vì layer ngang (Controllers/Services/Repositories). Mỗi feature là một slice khép kín chứa Endpoint + Request + Response + Validator + Handler
...
- **Auth:** JWT-based, dual issuer (own + host), BCrypt password hashing, middleware validate tự động
...
|----|-------------|----------|
| FR-01.1 | Login bằng username + password qua POST /api/auth/login (LoginEndpoint), trả về JWT | P0 |
| FR-01.2 | JWT lưu trong AuthContext (React Context), tự động gắn Authorization header cho mọi API call | P0 |
| FR-01.3 | Auth state lưu trong Zustand + AuthContext, tự redirect về /login nếu chưa auth hoặc JWT hết hạn | P0 |
| FR-01.4 | Hiển thị thông tin user: họ tên, phòng ban, chức vụ, role trong header | P1 |
| FR-01.5 | Logout xóa JWT + state, redirect về login | P0 |
| FR-01.6 | Chỉ user có `is_active = true` mới đăng nhập được | P0 |
| FR-01.7 | JWT có expiry 8h, middleware tự động trả về 401 khi token hết hạn | P0 |
| FR-01.8 | Dual auth support: nhận JWT từ host website qua postMessage → exchange token qua ExchangeEndpoint | P0 |

**Verification:** Test login với username/password đúng, sai, user bị disable. Test JWT hết hạn → 401. Test exchange token từ host JWT.
...
**Verification:** Login từng role, kiểm tra số liệu KPI khớp với dữ liệu thực tế trong DB.
...
| ID | Requirement | Priority |
|----|-------------|----------|
| FR-03.1 | Form: chọn loại phép (dropdown từ `leave_types` active), ngày start/end, lý do, người phê duyệt | P0 |
...
| FR-03.5 | Danh sách người phê duyệt lọc từ `approval_config` theo loại phép đã chọn: hiển thị approver role + cùng phòng ban nếu LD.PCM | P0 |
...
leave_types 1---* approval_config
employees   1---1 approval_config (approved_by)
```
...
|-------|---------|------------|
| `departments` | Phòng ban | id, name (unique), code |
| `employees` | Nhân viên | id, username (unique), password_hash, full_name, department_id (FK), role (enum: CB.PCM/LD.PCM/GD.PGD/QTHT), is_active |
| `leave_types` | Loại phép | id, name (unique), code (unique), default_days, is_active |
| `leave_requests` | Đơn nghỉ phép | id, employee_id (FK), leave_type_id (FK), start/end_date, total_days, status (pending/approved_leader/approved_director/rejected/cancelled), approved_by (FK employees) |
| `leave_balances` | Số dư phép | id, employee_id (FK), leave_type_id (FK), year, total_days, used_days, UNIQUE(employee_id, leave_type_id, year) |
| `leave_config` | Cấu hình | id, config_key (unique), config_value, description |
| `approval_config` | Cấu hình duyệt | id, leave_type_id (FK), approval_level, approver_role |

### 4.3 Auth & Security Model

- Auth: JWT-based với 2 issuer — app issuer (HS256, secret key) cho login trực tiếp + host issuer (RS256, public key validation) cho embed mode
- JWT claims: `{ sub: employee_id, role: "CB.PCM|LD.PCM|GD.PGD|QTHT", department_id: "...", exp: ..., iss: "app|host" }`
- Password: BCrypt hash (cost factor 12), không lưu plaintext
- Authorization: Role-based trong từng FastEndpoints endpoint — kiểm tra JWT claims + role trước khi thực thi business logic
- Middleware: JwtMiddleware validate token, extract claims → HttpContext.Items cho endpoint handler sử dụng
...
| Unique employee+leave_type+year | Database unique index |
| JWT expiry (8h) | JWT exp claim, middleware check |
| Role-based endpoint access | FastEndpoints `[Authorize]` attribute + role policy |
...
| Password hashing | **Fixed:** BCrypt hash (cost 12) trong SQL Server |
| JWT authentication | FastEndpoints middleware, dual issuer (HS256 + RS256) |
| Role-based API authorization | `[Authorize(Roles = "...")]` attribute per endpoint |
| Input validation | FluentValidation trong FastEndpoints pipeline (auto-validate) |
| HTTPS | Dependent on hosting / reverse proxy |
| Input sanitization | SQL parameterization via Dapper, FluentValidation rules |
...
|----|-----------|---------------------|---------------|
| FR-01 | Integration + Manual | POST /api/auth/login valid/invalid/disabled user, JWT expiry | JWT trả về đúng claims, 401 khi sai/expired |
...
| FR-09 | Integration | Verify violation calculation, filter period | Overage = total_used - 12, per employee |
| FR-10 | Integration | CRUD leave types, approval configs, general settings | Đúng persist trong DB |

### 6.2 Test Data Requirements
- Tối thiểu 3 departments
- Mỗi role ít nhất 1 employee
- Đơn với tất cả status: pending, approved_leader, approved_director, rejected, cancelled
- Ít nhất 1 employee vượt 12 ngày/năm để test violations
...
| Không có rate limiting trên login | **MEDIUM** | Implement login attempt tracking / cooldown qua FastEndpoints PreProcessor |
